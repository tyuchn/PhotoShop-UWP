...
namespace project.DrawingObjects
{
    public class CropUI : IDrawingUI
    {
        ...
        public void Draw(CanvasDrawingSession graphics, float scale)
        {
            ...
            graphics.FillRectangle(Region, Color.FromArgb(100, 0xD0, 0xD0, 0xD0));
            graphics.DrawRectangle(Region, DrawColor, stickness); //矩形
            if (Width > 50 && Height > 50)  //当满足条件时  绘制九宫格
            {
                graphics.DrawLine((float)Left, (float)(Top + (Height / 3)), (float)(Left + Width), (float)(Top + Height / 3), Colors.Pink, 0.3f, style);
                graphics.DrawLine((float)Left, (float)(Top + (Height * 2 / 3)), (float)(Left + Width), (float)(Top + Height * 2 / 3), Colors.Pink, 0.3f, style);
                graphics.DrawLine((float)(Left + Width / 3), (float)Top, (float)(Left + Width / 3), (float)(Top + Height), Colors.Pink, 0.3f, style);
                graphics.DrawLine((float)(Left + Width * 2 / 3), (float)Top, (float)(Left + Width * 2 / 3), (float)(Top + Height), Colors.Pink, 0.3f, style);
            }
